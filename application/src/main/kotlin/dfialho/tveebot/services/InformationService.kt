...
class InformationService(private val repository: TrackerRepository) : Service {
     ...
     */
    fun listAllTVShows(): List<TVShowEntity> = repository.findAllTVShows()

    /**
     * Returns a list containing every TV show currently being tracked.
     */
    fun listTrackedTVShows(): List<TVShowEntity> = repository.findTrackedTVShows()

    /**
     * Returns a list containing every TV show currently NOT being tracked.
     */
    fun listNonTrackedTVShows(): List<TVShowEntity> = repository.findNotTrackedTVShows()

    /**
     * Returns a map associating each TV show to its episodes. TV shows without any episodeFile are not included in the
     * returned map.
     */
    fun listEpisodesByTVShow(): Map<ID, List<EpisodeEntity>> = repository.findEpisodesByTVShow().mapKeys { it.key.id }
